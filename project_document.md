...
When running the application on a network server (such as Ubuntu), you must bind the services to your local network IP (e.g. `192.168.48.235`) so they can communicate across devices:

1. **Configure Backend `.env`** (`/server/.env`):
   ```env
   PORT=5000
   MONGODB_URI=your_mongodb_uri
   FRONTEND_URL=http://192.168.48.235:5173
   ```
2. **Configure Frontend `.env`** (`/client/.env`):
   ```env
   VITE_API_URL=http://192.168.48.235:5000/api/v1
   ```
3. **Run Backend in Background** (via terminal):
   ```bash
   cd ~/Kmg-training-management-system-clean/server
   nohup npm run dev > backend.log 2>&1 &
   ```
4. **Run Frontend in Background** (exposed to the network):
   ```bash
   cd ~/Kmg-training-management-system-clean/client
