...
const BulkUploadPage = () => {
  ...
  const handleDownloadTemplate = async (format) => {
    try {
      const response = await uploadApi.downloadTemplate(format);
  ...
  const startUploadAndValidate = async () => {
    ...
    try {
      ...
      const response = await uploadApi.uploadTrainingFile(selectedFile);
      const { batchId: serverBatchId } = response.data.data;
      setBatchId(serverBatchId);
      toast.info('File uploaded. Parsing records in background...');
    ...
    try {
      ...
      if (details.status === 'completed' || details.status === 'failed') {
        ...
        if (details.status === 'completed') {
          toast.success('Bulk import validation processing complete!');
        } else {
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <PageTitle title="Bulk Training Upload" subtitle="Import training records in bulk via CSV or Excel worksheets" />
        
        <Link
          to="/bulk-upload/history"
          className="inline-flex items-center space-x-1.5 px-3.5 py-2 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-850 rounded-xl text-xs font-bold shadow-sm transition-all"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-850/50 rounded-2xl p-6 shadow-sm space-y-4">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              To upload records, please download our spreadsheet template. The spreadsheet contains formatting validations. Do not delete or rename the header columns.
            </p>
          ...
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200/40 dark:border-slate-800/40 rounded-2xl p-5 space-y-3">
            ...
            <ul className="space-y-2 text-xs text-slate-500 dark:text-slate-400 list-disc list-inside">
              <li><strong>Staff Number:</strong> must exist in Staff Master list</li>
              <li><strong>Module Number:</strong> alphanumeric only</li>
        ...
        <div className="space-y-6">
          ...
          {uploading && (
            <div className="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-850/50 rounded-2xl p-6 shadow-sm space-y-4 text-center">
              ...
              <div className="space-y-1">
                <h3 className="font-bold text-slate-900 dark:text-white text-sm">Processing Spreadsheet Rows</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Validating fields format, looking up staff master records, and querying compound unique indexes...
                </p>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/50 dark:border-slate-850/50 text-center space-y-1 shadow-sm">
                  ...
                  <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Duplicate Duplicates</p>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {batchDetail.errorCount + batchDetail.duplicateCount > 0 && (
                    <button
                      ...
                      className="inline-flex items-center space-x-2 px-4 py-2 bg-red-600 hover:bg-red-750 text-white font-bold rounded-xl text-xs transition-all shadow-sm"
                    >
                      <Download className="w-4 h-4" />
                      <span>Download Error Spreadsheet</span>
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="inline-flex items-center space-x-1.5 px-4 py-2 border border-slate-250 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:bg-slate-55 rounded-xl text-xs font-bold transition-all shadow-sm"
                  >
